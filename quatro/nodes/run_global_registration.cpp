...
int main(int argc, char **argv) {
    ...
    string src_path = quatro_path + "/materials/001000.bin";
    string tgt_path = quatro_path + "/materials/001001.bin";
    *srcRaw = *getCloud(src_path);
    //*tgtRaw = *getCloud(tgt_path);
    *tgtRaw = *getCloud(src_path); // load the same cloud as source, and translate it in the next step
    Eigen::Matrix4d gt_transform;
    double angle = 0.087; //about 5 degrees
    gt_transform << std::cos(angle), std::sin(-angle), 0., 3.,
                    std::sin(angle), std::cos(angle), 0., 3.,
                    0., 0., 1., 0.,
                    0., 0., 0., 1.;
    pcl::transformPointCloud(*tgtRaw, *tgtRaw, gt_transform); // translate it
...
//    Eigen::Matrix<double, 3, Eigen::Dynamic> tgt_matched = fpfhmanager.getTgtMatched();
    ...
    *tgtMatched = fpfhmanager.getTgtKps();
    ...
    std::cout << setprecision(4) << "Estimated transform: translation: " << output(12) << ", " << output(13) << ", " << output(14) << " yaw " << std::acos(output(0))  << " (degree: " << std::acos(output(0))*180.0/3.1415 << ")" << std::endl;

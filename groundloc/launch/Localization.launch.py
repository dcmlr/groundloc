...
def setup(context):
	...
	model_file = launch.substitutions.LaunchConfiguration('model_file').perform(context)
	...
	if dataset_value == "kitti":
		...
		# select correct model file for the kitti dataset
		if model_file == '':
	...
	if dataset_value == "helipr":
		...
		# select correct model file for corresponding HeLiPR sensor
		if model_file == '':
			model_file = os.path.join(
		        get_package_share_directory('groundloc'),
		        'res',
		        'models',
		        sensor_value.lower() + '_finetuned_sejong3_15eps.pt'
		    )
	...
	if not os.path.isfile(model_file):
		print(f"ERROR: Could not find R2D2 file: {model_file}")
	...
	return [map_utm_tf,
		...
		launch_ros.actions.Node(
                                package='quatro',
								executable='quatro_node_cuda',
								name='quatro_node_cuda',
								output='screen',
								parameters=[
									...
									{'bev_registration/model_file': model_file},
									{'bev_registration/log_all_matchings': launch.substitutions.LaunchConfiguration('log_all_matchings')},
									{'bev_registration/use_cuda': launch.substitutions.LaunchConfiguration('use_gpu')},
...
def generate_launch_description():
	...
	correction_factor = launch.actions.DeclareLaunchArgument(
		...
		default_value='15.0',
		description="How many inliers are needed for full matching confidence",
	)
	...
	# R2D2 traced model file
	model_file = launch.actions.DeclareLaunchArgument(
		name="model_file",
		default_value='',
		description="Path to the traced R2D2 model file",
	)

	# use gpu for inference
	use_gpu = launch.actions.DeclareLaunchArgument(
		name="use_gpu",
		default_value='True',
